...
#a galerkian equation for every x and y
for i in range(0,k): #for every haar coefficient
    A[0,0] = 0
    B[0,0] = 0
    for j in range(0,k):
        A[0,0] = A[0,0] + gal1(i,coloc[j]) #these calculate for every t as specified by collocation
...
c1 = (x-0.3)**2 + (y-0.4)**2 - (0.11)**2
c2 = (x-0.6)**2 + (y-0.5)**2 - (0.1)**2 
c3 = (x-0)**2 + (y-0.3)**2 - (0.16)**2 
c4 = (x-0.7)**2 + (y-0.7)**2 - (0.1)**2 
...
c1 = plt.Circle((0.3,0.4),0.11,fill = False)
